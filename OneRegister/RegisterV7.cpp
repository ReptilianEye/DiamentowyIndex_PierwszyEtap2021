        ...
        if (sprawdzaCzyLiczbaJestPosortowana(liczbaRobocza))
        {
            ...
            while (prev_permutation(liczbaDoPermutacji.begin(), liczbaDoPermutacji.end()))
            {
                ...
                if (!visited[liczbaDoOdwiedzenia])
                    ...
                    {
                        ZerujeLiczbeZSortowaniem(stoll(liczbaDoPermutacji), iloscOperacji + 1 + abs(liczba - liczbaRobocza), 'p' + spisOperacjiNaTymPoziomie + spisOperacji);

                        // ZerujeLiczbeZSortowaniem(liczbaDoOdwiedzenia, iloscOperacji + 1 + spisOperacjiNaTymPoziomie.length(), to_string(liczbaRobocza) + "->" + liczbaDoPermutacji + 'p' + spisOperacjiNaTymPoziomie + spisOperacji);

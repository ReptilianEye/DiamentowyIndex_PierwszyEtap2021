...
string *ZnaneOdpowiedzi = new string[200000]{""};
...
string ZerujeLiczbeZSortowaniem(long long liczba, int iloscOperacji, string spisOperacji)
{
    ...
    int najblizszyKwadrat = pow(round(sqrt(liczba)), 2);
    int odlegloscDoNajblizszego = abs(liczba - najblizszyKwadrat);
    string najmniejszaOdpowiedzZPermutacji = string(10000, 'a');

    for (int i = 0; i <= odlegloscDoNajblizszego; i++)
    {
        // if (i == 52 && spisOperacji == "")
        // cout << 'a' << endl;
        for (int j = 0; j < 2; j++)
        {
            int liczbaRobocza;
                    ...
                    if (liczbaDoOdwiedzenia != liczbaRobocza)
                        if (!visited[liczbaDoOdwiedzenia])
                            if (stringDoPermutowania[0] != '0')
                            {
                                ...
                                string odpowiedz = ZerujeLiczbeZSortowaniem(liczbaDoOdwiedzenia, iloscOperacji + 1 + spisOperacjiNaTymPoziomie.length(), 'p' + spisOperacjiNaTymPoziomie + spisOperacji);
                                if (odpowiedz[0] != 'b')
                                {
                                    if (najmniejszaOdpowiedzZPermutacji.length() > odpowiedz.length())
                                    {
                                        najmniejszaOdpowiedzZPermutacji = odpowiedz;
                                        if (ZnaneOdpowiedzi[liczbaDoOdwiedzenia].length() > najmniejszaOdpowiedzZPermutacji.length() || ZnaneOdpowiedzi[liczbaDoOdwiedzenia] == "")
                                            ZnaneOdpowiedzi[liczbaDoOdwiedzenia] = najmniejszaOdpowiedzZPermutacji;
                                    }
    ...
    spisOperacji = 's' + spisOperacji;
    iloscOperacji++;
    string odpowiedzZKwadratu = ZerujeLiczbeZSortowaniem(liczba, iloscOperacji, spisOperacji);

    visited[oryginalnaLiczba] = false;
    if (odpowiedzZKwadratu[0] != 'b')
    {
        if (ZnaneOdpowiedzi[liczba].length() > odpowiedzZKwadratu.length() || ZnaneOdpowiedzi[liczba] == "")
            ZnaneOdpowiedzi[liczba] = odpowiedzZKwadratu;
        if (odpowiedzZKwadratu.length() > najmniejszaOdpowiedzZPermutacji.length() && najmniejszaOdpowiedzZPermutacji[0] != 'b')
            return najmniejszaOdpowiedzZPermutacji;
        else
            return odpowiedzZKwadratu;
    } //----
    else
        return najmniejszaOdpowiedzZPermutacji;
}
